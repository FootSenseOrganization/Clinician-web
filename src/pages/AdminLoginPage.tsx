...
export default function AdminLoginPage() {
  ...
  useEffect(() => {
    ...
    if (role === "admin") {
      ...
      // Clinician trying admin login — redirect to clinician dashboard
      navigate("/dashboard", { replace: true });
    } else if (isAuthenticated && !role) {
      setError(
        `No admin account found for ${authEmail}. This portal is restricted to authorised administrators.`,
      );
    }
  ...
  return (
    <div className="sidebar-gradient flex min-h-screen flex-col items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <Link
          to="/"
          className="group mb-4 inline-flex items-center gap-2 text-sm font-medium text-sidebar-foreground/70 transition-colors duration-200 hover:text-sidebar-accent-foreground"
        >
        ...
        <div className="mt-8 space-y-4">
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-3 rounded-lg bg-white/10 border border-white/10 px-4 py-3 text-sm font-semibold text-sidebar-accent-foreground transition-all duration-200 hover:-translate-y-0.5 hover:bg-white/15 disabled:opacity-60 disabled:translate-y-0"
          >
